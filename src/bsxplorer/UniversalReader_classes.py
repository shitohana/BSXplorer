...
from .UniversalReader_batches import (
    UniversalBatch,
    ARROW_SCHEMAS,
    ReportTypes,
    REPORT_TYPES_LIST,
...
class UniversalReplicatesReader(object):
    ...
    def __init__(
        ...
        if any(map(lambda reader: reader.report_type in ["bedgraph"], self.readers)):
            warnings.warn(
                "Merging bedGraph may lead to incorrect results. Please, use other report types."
            )
    ...
    def __next__(self):
        ...
        for chrom in pack_seen_chroms:
            ...
            other = [
                key
                for key in batches_data.keys()
                if key != list(batches_data.keys())[0]
            ]
            for key in other:
    ...
    def _convert_to_full(df: pl.DataFrame):
        ...
        ).select(UniversalBatch.colnames())
