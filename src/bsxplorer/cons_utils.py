...
def render_template(template: str | Path, params: dict, output: str | Path):
    with open(template) as template_file:
        j2_template = Template(template_file.read())

        with open(output, "w") as output_file:
            output_file.write(j2_template.render(params))
...
def menu_parser():
    parser = argparse.ArgumentParser(
        ...
        epilog="Call bsx help to get more information about features."
    )
    parser.add_argument("prog", help="Program to use", choices=["metagene", "levels", "category", "help"])
    return parser
...
def metagene_parser():
    ...
    parser.add_argument(
        ...
        default=f"Metagene_Report_{time.strftime('%d-%m-%y_%H-%M-%S')}",
        metavar="NAME",
    )
    parser.add_argument(
        "--dir",
        help="Output and working dir",
        default=Path.cwd(),
        metavar="DIR",
        type=_working_dir,
    )
    ...
    parser.add_argument(
        "-s",
        "--sumfunc",
        help="Summary function to calculate density for bin with.",
        type=str,
        default="wmean",
        choices=["wmean", "mean", "median", "min", "max", "1pgeom"],
    )
    parser.add_argument(
        "-u",
        "--ubin",
        help="Number of windows for upstream region",
        type=int,
        default=50,
    )
    parser.add_argument(
        "-d",
        "--dbin",
        help="Number of windows for downstream downstream",
        type=int,
        default=50,
    )
    parser.add_argument(
        "-b",
        "--bbin",
        help="Number of windows for body region",
        type=_bbin,
        default=100,
    )
    ...
    parser.add_argument(
        "-q",
        "--quantile",
        help="Quantile of most varying genes to draw on clustermap",
        type=_quantile,
        default=0.75,
    )
    parser.add_argument(
        "-C",
        "--confidence",
        help="Probability for confidence bands for line-plot. 0 if disabled",
        type=_quantile,
        default=0.95,
    )
    parser.add_argument(
        "-S", "--smooth", help="Windows for SavGol function.", type=float, default=10
    )
    parser.add_argument(
        "-H",
        help="Vertical resolution for heat-map",
        type=int,
        default=100,
        dest="vresolution",
    )
    ...
    parser.add_argument(
        "--separate_strands",
        help="Do strands need to be processed separately",
        action="store_true",
    )
    parser.add_argument(
        "--export",
        help="Export format for plots (set none to disable)",
        type=str,
        default="pdf",
    ...
    parser.add_argument(
        "--ticks",
        help="Names of ticks (- character should be escaped with double reverse slash)",
        type=_ticks,
        nargs=5,
    )
...
def chr_parser():
    ...
    parser.add_argument(
        "-w", "--window", help="Length of windows in bp", type=int, default=10**6
    )
    parser.add_argument(
        "-l",
        "--min_length",
        help="Minimum length of chromosome to be analyzed",
        type=int,
        default=10**6,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-S", "--smooth", help="Windows for SavGol function.", type=float, default=100
    )
...
class ConsoleScript(ABC):
    ...
    def _read_config(self, path) -> pl.DataFrame:
        ...
        report_args = (
            report_args.with_row_count("id")
            .with_columns([
                pl.col("report_file").map_elements(
                    lambda path: str(Path(path).expanduser().absolute())
                ),
                pl.col("genome_file").map_elements(
                    lambda path: str(Path(path).expanduser().absolute())
                ),
    ...
    def _validate_config(self, config: pl.DataFrame):
        ...
        if sum([t not in self._GENOME_TYPES for t in config["genome_type"]]) > 0:
            raise ValueError(
                f"Not all genome types are supported. (Supported: {self._GENOME_TYPES})"
            )

        # unsupported report type check
        if sum([t not in self._REPORT_TYPES for t in config["report_type"]]) > 0:
            raise ValueError(
                f"Not all genome types are supported. (Supported: {self._REPORT_TYPES})"
            )

        # unique genome for sample group check
        if (
            config.group_by("name")
            .agg(pl.col("genome_file").unique().len() != 1)["genome_file"]
            .sum()
            > 0
        ):
    ...
    def sample_list(self) -> list:
        mapping = (
            self.config.group_by("name")
            .agg([
                pl.col("report_file"),
                pl.col("report_type"),
                pl.col("genome_file").first(),
            ])
            .to_dicts()
        )
    ...
    def genome_mapping(self) -> dict:
        ...
        for row in grouped.to_dicts():
            ...
            kwargs = dict(
                min_length=row["min_length"], flank_length=row["flank_length"]
            )
            if row["genome_type"] is None or row["genome_type"] in ["none", "all"]:
                annotation: pl.DataFrame = genome.all(**kwargs)
            else:
                annotation: pl.DataFrame = genome.other(
                    region_type=row["region_type"], **kwargs
                )
...
class MetageneScript(ConsoleScript):
    ...
    def main(self):
        ...
        for row in self.sample_list:
            report_files = (
                row["report_file"]
                if isinstance(row["report_file"], list)
                else list(row["report_file"])
            )
            report_types = (
                row["report_type"]
                if isinstance(row["report_type"], list)
                else list(row["report_type"])
            )
            ...
            for report_file, report_type in zip(report_files, report_types):
                ...
                if pca is not None:
                    pca.append_metagene(
                        report_metagene, Path(report_file).stem, row["name"]
                    )
                replicates.append(report_metagene)

            sample_metagenes.append(MetageneFiles(replicates).merge())
...
class CategoryScript(ConsoleScript):
    ...
    def _parser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            "--cytosine_p",
            help="P-value for binomial test to consider cytosine methylated",
            default=".05",
            type=float,
        )
        parser.add_argument(
            "--min_cov",
            help="Minimal coverage for cytosine to keep",
            default="2",
            type=int,
        )
        parser.add_argument(
            "--region_p",
            help="P-value for binomial test to consider region methylated",
            default=".05",
            type=float,
        )
        parser.add_argument(
            "--save_cat",
            help="Does categories need to be saved",
            default=True,
            type=bool,
    ...
    def main(self):
        ...
        for row in self.sample_list:
            ...
            if len(report_files) > 1:
                temp = init_tempfile(
                    self.args.dir, row["name"] + "_merged", delete=False, suffix=".txt"
                )
            ...
            region_pvalues = binom.region_pvalue(
                genome_mapping[row["name"]], self.args.cytosine_p, self.args.threads
            )
    ...
    def main(self):
        ...
        for row in self.sample_list:
            ...
            if merged_type not in ["parquet"]:
                kwargs |= dict(
                    block_size_mb=self.args.block_mb, use_threads=self.args.threads
                )
...
class Renderer:
    ...
    def _get_filters(self):
        if self.args.separate_strands:
            ...
            return [dict(context=context) for context in ["CG", "CHG", "CHH"]]
    ...
    def category_context_block(self, bm, im, um, filters, metadata):
        ...
        caption_text = "<p>Group regions count:</p>\n" + "\n".join([
            f"<p>{label} – "
            + ", ".join([
                f"{group}: {number}" for group, number in metadata[label].items()
            ])
            + " regions</p>"
            for label in metadata.keys()
        ])
        context_block = TemplateContext(heading=heading_text, caption=caption_text)
        ...
        for mfiles, mtype, plot_idx in zip([bm, im, um], subplot_titles, range(1, 4)):
            ...
            heat_map = mfiles.heat_map(
                nrow=self.args.vresolution, ncol=self.args.hresolution
            )
            ...
            if self.args.export not in ["none"]:
                self._save_mpl(
                    line_plot.draw_mpl(tick_labels=self.args.ticks),
                    name.format(type="line_plot"),
                )
                self._save_mpl(
                    heat_map.draw_mpl(tick_labels=self.args.ticks),
                    name.format(type="heat_map"),
                )
            ...
            # Line plot
            line_plot.draw_plotly(
                lp_fig, tick_labels=self.args.ticks, fig_rows=1, fig_cols=plot_idx
            )
            # Heat map
            for count, (hm_data, label) in enumerate(
                zip(heat_map.data, metadata.keys())
            ):
                HeatMap(hm_data).draw_plotly(
                    hm_fig,
                    tick_labels=self.args.ticks,
                    row=count + 1,
                    col=plot_idx,
                    title=label + mtype,
                )
        ...
        context_block.plots.append(
            TemplatePlot(
                "Line plot",
                self._p2html(
                    lp_fig, include_plotlyjs=self.__add_plotlyjs, default_width="80%"
                ),
            )
        )
        ...
        context_block.plots.append(
            TemplatePlot(
                "Heatmap",
                self._p2html(
                    hm_fig, include_plotlyjs=self.__add_plotlyjs, default_width="80%"
                ),
            )
        )
        context_block.plots.append(
            TemplatePlot(
                "Box plot", self._p2html(bp_fig, include_plotlyjs=self.__add_plotlyjs)
            )
        )

        return context_block

    def metagene_context_block(
        self, filtered: MetageneFiles, filters: dict, draw_cm: bool = True
    ):
        filter_name = self._format_filters(filters)
        name = filter_name + "_{type}"

        context_block = TemplateContext(heading=f"Context {filter_name}")
        ...
        line_plot = filtered.line_plot(
            merge_strands=not self.args.separate_strands,
            smooth=self.args.smooth,
            confidence=self.args.confidence,
            stat="mean",
        )
        heat_map = filtered.heat_map(
            nrow=self.args.vresolution, ncol=self.args.hresolution
        )
        ...
        context_block.plots.append(
            TemplatePlot(
                "Line plot",
                self._p2html(
                    line_plot.draw_plotly(tick_labels=self.args.ticks),
                    include_plotlyjs=self.__add_plotlyjs,
                ),
            )
        )
        ...
        context_block.plots.append(
            TemplatePlot(
                "Heat map",
                self._p2html(
                    heat_map.draw_plotly(tick_labels=self.args.ticks),
                    include_plotlyjs=self.__add_plotlyjs,
                ),
            )
        )
        ...
        context_block.plots.append(
            TemplatePlot(
                "Box plot",
                self._p2html(
                    box_plot.draw_plotly(), include_plotlyjs=self.__add_plotlyjs
                ),
            )
        )
        collect()
    ...
    def metagene(self, metagene_files: MetageneFiles, pca: PCA, draw_cm: bool = True):
        ...
        if pca is not None:
            try:
                pca_plot_data = self._p2html(
                    pca.draw_plotly(), include_plotlyjs=self.__add_plotlyjs
                )
                pca_plot = TemplatePlot(data=pca_plot_data)
                pca_block = TemplateContext(
                    heading="PCA plot for all replicates", caption="", plots=[pca_plot]
                )
        ...
        for filters in filters_list:
            ...
            context_block = self.metagene_context_block(
                filtered_metagenes, filters, draw_cm
            )
    ...
    def category(self, metagene_files: MetageneFiles, region_pvalues: dict):
        ...
        for filters in filters_list:
            ...
            for sample, label in zip(
                filtered_metagenes.samples, filtered_metagenes.labels
            ):
                save_name = (
                    self.args.dir / f"{label}_{self._format_filters(filters)}"
                    if self.args.save_cat
                    else None
                )
            ...
            bm_metagene_files = MetageneFiles(
                bm_metagenes, [label + "_BM" for label in filtered_metagenes.labels]
            )
            im_metagenes_files = MetageneFiles(
                im_metagenes, [label + "_IM" for label in filtered_metagenes.labels]
            )
            um_metagene_files = MetageneFiles(
                um_metagenes, [label + "_UM" for label in filtered_metagenes.labels]
            )

            html_body.context_reports.append(
                self.category_context_block(
                    bm_metagene_files,
                    im_metagenes_files,
                    um_metagene_files,
                    filters,
    ...
    def chr_levels(self, chr_levels_list: list[ChrLevels], labels: list[str]):
        ...
        for filters in filters_list:
            context_report = TemplateContext(
                heading=f"Context {self._format_filters(filters)}"
            )
            ...
            context_report.plots.append(
                TemplatePlot(
                    "Line plot",
                    lp_fig.to_html(
                        full_html=False, include_plotlyjs=self.__add_plotlyjs
                    ),
                )
            )
            if self.__add_plotlyjs:
