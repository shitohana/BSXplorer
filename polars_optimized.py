...
def read_bismark_full(file: str, genes: pl.DataFrame, flank_windows=500, gene_windows=2000) -> pl.DataFrame:
    with pl.StringCache():
        ...
        bismark_lazy = (
            pl.scan_csv(
                file,
                separator='\t', low_memory=False, has_header=False,
                new_columns=['chr', 'position', 'strand', 'count_m', 'count_um', 'context', 'additional'],
                dtypes={
                    'position': pl.Int32,
                    'count_m': pl.Int8,
                    'count_um': pl.Int8,
            ...
            # drop explicit context column
            .drop('additional')
            # filter unregistered cytosines
            ...
            .join_asof(
                genes.lazy(), left_on='position', right_on='upstream', by=['chr', 'strand']
            )
            ...
            .with_columns(
                ...
                pl.when(
                    ...
                    -(((pl.col('start') - pl.col('position')) / (
                            pl.col('start') - pl.col('upstream'))) * flank_windows).floor()
                )
                # gene body
                .when(
                    ...
                    (((pl.col('position') - pl.col('start')) / (
                            pl.col('end') - pl.col('start') + 1)) * gene_windows).floor()
                )
                # downstream
                .when(
                    (pl.col('position') > pl.col('end'))
                ).then(
                    gene_windows + (((pl.col('position') - pl.col('end')) / (
                            pl.col('downstream') - pl.col('end'))) * flank_windows).floor()
                )
            ...
            .sort(['chr', 'strand', 'fragment'])
        )
        return bismark_lazy.collect()
